    ...
    bool deskewInfo()
    {
        ...
        if (imuQueue.empty())
        {
            RCLCPP_INFO(get_logger(),
                "Waiting IMU: queue EMPTY, scan=[%.6f, %.6f]",
                timeScanCur, timeScanEnd);
            return false;
        }

        double imuFirst = stamp2Sec(imuQueue.front().header.stamp);
        double imuLast  = stamp2Sec(imuQueue.back().header.stamp);

        if (imuFirst > timeScanCur || imuLast < timeScanEnd)
        {
            RCLCPP_INFO(get_logger(),
                "Waiting IMU: scan=[%.6f, %.6f], imu=[%.6f, %.6f]",
                timeScanCur, timeScanEnd, imuFirst, imuLast);
            return false;
        }
